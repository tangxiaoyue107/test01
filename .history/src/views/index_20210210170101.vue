    ...
    <div class="box">
      ...
      <div class="recent" style="display:flex;">
        <recent>
          ...
          <span slot="recent_more" @click="fruit_more()" class="recent_more">MORE</span>
          <div slot="content" v-for="(item,index) in this.fruit" :key='index'>
            <div class="recent_box">
              ...
              <div class="recent_text">
                <h1 @click="fruit_jump(item.pid)">{{item.p_title}}</h1>
              </div>
            </div>
              <hr>
          </div>
...
export default {
  ...
  created(){
    ...
    request({
      url:'/client/home/rg_paper',
        ...
        this.lasttime=res.data.timeList[0];
  ...
  methods:{
    fruit_more(){
      this.$router.push({path:'/fruit_show/article',query:{year:this.lasttime}});
    },
    recent_jump(router){
      this.$router.push(router);
    },
    fruit_jump(pid){
      this.$router.push({path:'/fruit_show/article/article_detail?',query:{cpid:pid}});
    },
    act_jump(status,nid){
      // console.log(status);
      if(status==0){
        ...
        this.$router.push({path:"/info/annouced/annouced_detail?",query:{cnid:nid}});
      }
    }
    }
  }
