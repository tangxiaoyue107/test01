<template>
  <div class="info">
      <righttitle><span slot="title">{{$t('m.public_resource')}}</span></righttitle>
      <div class="whole_box">
...
export default {
  ...
  methods:{
    info_detail(){
      this.$router.push('/info/public_resource/public_resource_detail');
    }
  },
  data(){
    return {
  ...
  methods:{
    change(content){
      request({
        url:"/client/news/catalogue/2/"+content,
      }).then(res=>{
        var data = res.data;
          // console.log(data);
          this.box = data.data;
    ...
    info_detail(nid){
      this.$router.push('/info/public_resource/public_resource_detail?'+nid);
    },
    cpnclick(val){
      this.change(val);
    }
